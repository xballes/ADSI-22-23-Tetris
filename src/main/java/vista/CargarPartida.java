...
public class CargarPartida extends JFrame implements Ventana {
	
	private static CargarPartida puntero;
	...
	public static void visibilizar() {
		CargarPartida.puntero = new CargarPartida();
	...
	private CargarPartida () {
		...
		this.titulo = new JLabel("Cargar Partida", SwingConstants.CENTER);
		...
		for (int i = 0; i !=2; i++) {
			...
			case 1: val = "                 Fecha de la partida"; break;
		...
		for (int i = 0; i != 2; i++) {
			String val = null;
			switch (i) {
			case 0: val = "Cargar Partida"; break;
	...
	private class Accion1 implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) { //CARGAR PARTIDA
			boolean Resultado=GestorGuardado.getInstancia().cargarPartida(campos[0].getText(),campos[1].getText());		
			if(Resultado!=false) {
